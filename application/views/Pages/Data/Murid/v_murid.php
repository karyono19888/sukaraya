	...
	<div id="root">
		...
		<main>
			<div class="container">
				...
				<div class="page-title-container">
					<div class="row">
						...
						<div class="col-12 col-md-7">
							<h1 class="mb-0 pb-0 display-4" id="title">Murid</h1>
				...
				<!-- Content Start -->
				<a href="<?= base_url('Pages/Data/Murid/ShowTambah');?>" type="button" class="btn btn-outline-primary btn-icon btn-icon-start mb-3" id="Tambah" onclick="Tambah()">
									...
									<table id="table" class="table table-hover">
										<thead>
											<tr>
												...
												<th class="text-muted text-uppercase">Jenis Kelamin</th>
												<th class="text-muted text-uppercase">Tanggal Lahir</th>
												...
												<th class="text-muted text-uppercase">Wali</th>
												<th class="text-muted text-uppercase">No Wali</th>
										...
										<tbody>
											<?php
											$i=1;
													foreach($murid->result_array() as $a):
												?>
											<tr>
												<td><?= $i++; ?></td>
												...
												<td><?= $a['murid_nama']; ?></td>
												<td><?= $a['murid_jk']; ?></td>
												<td><?= $a['murid_tgl_lahir']; ?></td>
												<td><?= $a['pend_nama']; ?></td>
												<td><?= $a['kelas_nama']; ?></td>
												<td><?= $a['kel_nama']; ?></td>
												<td><?= $a['murid_wali']; ?></td>
												<td><?= $a['murid_no_wali']; ?></td>
												<td>
												...
												<td>
													<a href="<?= base_url('Pages/Data/Murid/ShowEditMurid/'.$a['murid_id'].'')?>" class="btn btn-sm btn-icon btn-icon-start btn-outline-warning ms-1 Edit"
														type="button">
													...
													<button class="btn btn-sm btn-icon btn-icon-start btn-outline-danger ms-1 Hapus"
														type="button" data-id="<?= $a['murid_id'];?>">
		...
		$(document).on("click", ".Hapus", function() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						type: 'POST',
						url: '<?= site_url('Pages/Data/Murid/Hapus') ?>',
						data: {
							id: id
						},
