	...
	<div id="root">
				...
				<div class="page-title-container">
					<div class="row">
						...
						<div class="col-12 col-md-7">
							<h1 class="mb-0 pb-0 display-4" id="title">Kelas</h1>
											...
											<div class="col">
												<div class="row gx-2 d-flex align-content-center">
													<div class="col-12 col-xl d-flex">
														<div class="d-flex align-items-center lh-1-25">Total Kelas</div>
													</div>
									...
									<table id="table" class="table table-hover">
										...
										<tbody>
											...
											<tr>
												...
												<td><?= $a['kelas_nama']; ?></td>
												<td>
													<?php if($a['kelas_status'] =="Aktif"): ?>
													<span class="badge bg-primary text-uppercase"><?= $a['kelas_status']; ?></span>
													<?php else: ?>
													<span class="badge bg-danger text-uppercase"><?= $a['kelas_status']; ?></span>
													<?php endif; ?>
												</td>
												<td>
													...
													<button class="btn btn-sm btn-icon btn-icon-start btn-outline-danger ms-1 Hapus"
														type="button" data-id="<?= $a['kelas_id'];?>">
						...
						<div class="modal-body">
							<form method="POST" id="formModalUser">
								...
								<div class="mb-1">
									...
									<select id="kelas_status" name="kelas_status" class="form-control">
		...
		$(document).on("click", "#ModalButtonSimpan", function() {
			if (validasi()) {
				...
				$.ajax({
					type: 'POST',
					url: '<?= site_url('Master/Kelas/Tambah') ?>',
					data: data,
					beforeSend: function() {
					...
					success: function(response) {
						...
						setTimeout(() => {
							window.location.assign('<?= site_url("Master/Kelas") ?>');
						}, 2000);
					}
				});
			}
		...
		function validasi() {
			let kelas_nama = document.getElementById("kelas_nama").value;
			let kelas_status = document.getElementById("kelas_status").value;
			if ((kelas_nama == "") || (kelas_status == "")) {
				if (kelas_status == "") {
					notif("Status");
				}
				if (kelas_nama == "") {
		...
		$(document).on("click", ".Edit", function() {
			...
			$.ajax({
				type: 'POST',
				url: '<?= site_url('Master/Kelas/View'); ?>',
				data: {
					id: id
				},
				success: function(response) {
					...
					if (data.success) {
						...
						$('#kelas_status').val(data.kelas_status);
					} else {
						Swal.fire({
							icon: 'warning',
							title: 'Warning',
		...
		$(document).on("click", ".Hapus", function() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						type: 'POST',
						url: '<?= site_url('Master/Kelas/Hapus') ?>',
						data: {
							id: id
						},
						success: function(response) {
