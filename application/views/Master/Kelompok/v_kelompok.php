...
<body>
	<div id="root">
		...
		<main>
				...
				<div class="page-title-container">
					<div class="row">
						...
						<div class="col-12 col-md-7">
							<h1 class="mb-0 pb-0 display-4" id="title">Kelompok</h1>
											...
											<div class="col">
												<div class="row gx-2 d-flex align-content-center">
													<div class="col-12 col-xl d-flex">
														<div class="d-flex align-items-center lh-1-25">Total Kelompok</div>
													</div>
									...
									<table id="table" class="table table-hover">
										...
										<tbody>
											...
											<tr>
												...
												<td><?= $a['kel_nama']; ?></td>
												<td>
													<?php if($a['kel_status'] =="Aktif"): ?>
													<span class="badge bg-primary text-uppercase"><?= $a['kel_status']; ?></span>
													<?php else: ?>
													<span class="badge bg-danger text-uppercase"><?= $a['kel_status']; ?></span>
													<?php endif; ?>
												</td>
												<td>
													<button class="btn btn-sm btn-icon btn-icon-start btn-outline-warning ms-1 Edit"
														type="button" data-bs-toggle="modal" data-bs-target="#ModalKelompok"
														data-id="<?= $a['kel_id'];?>">
			...
			<div class="modal fade" id="ModalKelompok" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
				role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered">
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title" id="titleTambah">Tambah Kelompok Baru</h5>
							<h5 class="modal-title" id="titleEdit">Edit Kelompok</h5>
							...
							<form method="POST" id="formModalUser">
								<div class="mb-1">
									...
									<input type="text" class="form-control" id="kel_nama" name="kel_nama"
										placeholder="Contoh : Sukaraya 1" />
								</div>
								<div class="mb-1">
									<label class="form-label">Status</label>
									<select id="kel_status" name="kel_status" class="form-control">
		...
		$(document).on("click", "#ModalButtonSimpan", function() {
			if (validasi()) {
				...
				$.ajax({
					type: 'POST',
					url: '<?= site_url('Master/Kelompok/Tambah') ?>',
					data: data,
					beforeSend: function() {
					...
					success: function(response) {
						...
						setTimeout(() => {
							window.location.assign('<?= site_url("Master/Kelompok") ?>');
						}, 2000);
					}
				});
			}
		...
		function validasi() {
			let kel_nama = document.getElementById("kel_nama").value;
			let kel_status = document.getElementById("kel_status").value;
			if ((kel_nama == "") || (kel_status == "")) {
				if (kel_status == "") {
					notif("Status");
				}
				if (kel_nama == "") {
		...
		$(document).on("click", ".Edit", function() {
			...
			$.ajax({
				type: 'POST',
				url: '<?= site_url('Master/Kelompok/View'); ?>',
				data: {
					id: id
				},
				success: function(response) {
					...
					if (data.success) {
						$('#kel_id').val(data.kel_id);
						$('#kel_nama').val(data.kel_nama);
						$('#kel_status').val(data.kel_status);
					} else {
						Swal.fire({
							icon: 'warning',
							title: 'Warning',
		...
		$(document).on("click", "#ModalButtonEdit", function() {
			if (validasiupdate()) {
				...
				$.ajax({
					type: 'POST',
					url: '<?= site_url('Master/Kelompok/Edit') ?>',
					data: data,
					beforeSend: function() {
		...
		$(document).on("click", ".Hapus", function() {
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						type: 'POST',
						url: '<?= site_url('Master/Kelompok/Hapus') ?>',
						data: {
							id: id
						},
						success: function(response) {
